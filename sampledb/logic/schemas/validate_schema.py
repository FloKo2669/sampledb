...
def validate_schema(
        ...
        *,
        ...
        parent_conditions: typing.Optional[typing.List[typing.Tuple[typing.List[str],
    ...
    _validate_title_and_tooltip_in_schema(
        schema, path, all_language_codes=all_language_codes, strict=strict)
    if 'style' in schema and not isinstance(schema['style'], str) and not (isinstance(schema['style'], dict) and all(isinstance(key, str) for key in schema['style'].keys()) and all(isinstance(value, str) or value is None for value in schema['style'].values())):
        raise ValidationError(
            'style must be a string or a dict of string keys and string or null values', path)
    if 'conditions' in schema:
        if parent_conditions is not None:
            ...
            raise ValidationError(
                'only schemas with an object parent may contain conditions', path)
...
def _validate_note_in_schema(
    ...
    if 'note' in schema and isinstance(schema['note'], dict):
        if 'en' not in schema['note']:
            raise ValidationError(
                'note must include an english translation', path)
        for lang_code in schema['note'].keys():
            if lang_code not in all_language_codes:
                raise ValidationError(
                    'note must only contain known languages', path)
        for note_text in schema['note'].values():
            ...
            if strict and re.match(r'^\s+$', note_text):
                raise ValidationError(
                    'note text must not be whitespace only', path)
...
def _validate_hazards_schema(
    ...
    valid_keys = {'type', 'title', 'note', 'dataverse_export',
                  'scicat_export', 'may_copy', 'style', 'tooltip'}
    required_keys = {'type', 'title'}
    schema_keys = set(schema.keys())
    invalid_keys = schema_keys - valid_keys
    if invalid_keys:
        raise ValidationError(
            f'unexpected keys in schema: {invalid_keys}', path)
    ...
    if path != ['hazards']:
        raise ValidationError(
            'GHS hazards must be a top-level entry named "hazards"', path)

    _validate_note_in_schema(
        schema, path, all_language_codes=all_language_codes, strict=strict)
...
def _validate_array_schema(
    ...
    :raise ValidationError: if the schema is invalid.
    """
    valid_keys = {'type', 'title', 'items', 'style', 'tooltip', 'minItems',
                  'maxItems', 'defaultItems', 'default', 'may_copy', 'conditions', 'note'}
    ...
    if has_min_items and has_max_items:
        if schema['minItems'] > schema['maxItems']:
            raise ValidationError(
                'minItems must be less than or equal to maxItems', path)
    if has_min_items and has_default_items:
        if schema['minItems'] > schema['defaultItems']:
            raise ValidationError(
                'minItems must be less than or equal to defaultItems', path)
    if has_default_items and has_max_items:
        if schema['defaultItems'] > schema['maxItems']:
            raise ValidationError(
                'defaultItems must be less than or equal to maxItems', path)
    validate_schema(schema['items'], path + ['[?]'], invalid_template_action_ids=invalid_template_action_ids,
                    strict=strict, all_language_codes=all_language_codes, root_schema=root_schema)
    if 'default' in schema:
        if has_default_items:
            raise ValidationError(
                'default and defaultItems are mutually exclusive', path)
        validate(schema['default'], schema, path +
...
def _validate_tags_schema(
    ...
    if missing_keys:
        raise ValidationError(f'missing keys in schema: {missing_keys}', path)
    if 'default' in schema:
        validate({'_type': 'tags', 'tags': schema['default']},
                 schema, path + ['(default)'], strict=strict)
    if 'dataverse_export' in schema and not isinstance(schema['dataverse_export'], bool):
        raise ValidationError('dataverse_export must be True or False', path)
    if 'scicat_export' in schema and not isinstance(schema['scicat_export'], bool):
        raise ValidationError('scicat_export must be True or False', path)
    if path != ['tags']:
        raise ValidationError(
            'Tags must be a top-level entry named "tags"', path)
...
def _validate_object_schema(
    ...
    try:
        ...
        raise ValidationError(
            'template must not recursively include itself', path)
    except InvalidTemplateIDError:
        raise ValidationError(
            'template must be the ID of a template action', path)

    if schema.get('template') is not None:
        invalid_template_action_ids = list(
            invalid_template_action_ids) + [typing.cast(int, schema['template'])]

    valid_keys = {'type', 'title', 'properties', 'propertyOrder', 'required', 'default',
                  'may_copy', 'style', 'template', 'recipes', 'note', 'show_more', 'workflow_show_more'}
    if not path:
    ...
    property_conditions: typing.List[typing.Tuple[typing.List[str],
                                                  typing.Dict[str, typing.Any]]] = []
    property_calculations: typing.List[typing.Tuple[typing.List[str],
                                                    typing.Dict[str, typing.Any]]] = []
    property_schemas = {}
    ...
    for condition_path, condition in property_conditions:
        if not isinstance(condition, dict) or not isinstance(condition.get('type'), str):
            raise ValidationError(
                'condition must be a dict containing the key type', condition_path)
    ...
    for calculation_path, calculation in property_calculations:
        calculations.validate_calculation(
            calculation, root_schema, calculation_path)
    ...
    if 'required' in schema:
        ...
        for i, property_name in enumerate(schema['required']):
            if property_name not in schema['properties']:
                raise ValidationError(
                    f'unknown required property: {property_name}', path)
            if property_name in schema['required'][:i]:
                raise ValidationError(
                    f'duplicate required property: {property_name}', path)
            ...
            if schema['properties'][property_name].get('conditions') and property_name == 'name' and path == []:
                raise ValidationError(
                    f'conditional required property: {property_name}', path)
    ...
    if not path:
        if 'name' not in schema['properties'] or schema['properties']['name']['type'] != 'text':
            raise ValidationError(
                'Schema must include a text property "name"', path)
        if schema['properties']['name'].get('multiline') or schema['properties']['name'].get('markdown'):
            raise ValidationError(
                'Object name must not be multiline or markdown text', path)
        if 'required' not in schema or 'name' not in schema['required']:
            raise ValidationError(
                '"name" must be a required property for the root object', path)
    ...
    if 'propertyOrder' in schema:
        ...
        for i, property_name in enumerate(schema['propertyOrder']):
            if property_name not in schema['properties']:
                raise ValidationError(
                    f'unknown propertyOrder property: {property_name}', path)
            if property_name in schema['propertyOrder'][:i]:
                raise ValidationError(
                    f'duplicate propertyOrder property: {property_name}', path)
    ...
    if 'displayProperties' in schema:
        ...
        for i, property_name in enumerate(schema['displayProperties']):
            if property_name not in schema['properties']:
                raise ValidationError(
                    f'unknown display property: {property_name}', path)
            if property_name in schema['displayProperties'][:i]:
                raise ValidationError(
                    f'duplicate displayProperties property: {property_name}', path)
    ...
    if 'batch_name_format' in schema:
        if not schema.get('batch', False):
            raise ValidationError(
                'batch must be True for batch_name_format to be set', path)
    ...
    if 'recipes' in schema:
        ...
        for i, recipe in enumerate(schema['recipes']):
            if 'name' not in recipe:
                raise ValidationError(
                    'missing recipe name', path + ['(recipes)', f'[{i}]'])
            if not isinstance(recipe['name'], str) and not isinstance(recipe['name'], dict):
                raise ValidationError(
                    'recipe name must be str or dict', path + ['(recipes)', f'[{i}]'])
            if isinstance(recipe['name'], dict):
                if 'en' not in recipe['name']:
                    raise ValidationError(
                        'recipe name must include an english translation', path + ['(recipes)', f'[{i}]'])
                for lang_code in recipe['name'].keys():
                    if lang_code not in all_language_codes:
                        raise ValidationError(
                            'recipe name must only contain known languages', path + ['(recipes)', f'[{i}]'])
                for name_text in recipe['name'].values():
                    if not isinstance(name_text, str):
                        raise ValidationError(
                            'recipe name must only contain text', path + ['(recipes)', f'[{i}]'])
            if 'property_values' not in recipe:
                raise ValidationError(
                    'missing property_values', path + ['(recipes)', f'[{i}]'])
            for property_name in recipe['property_values']:
                if property_name not in schema['properties'].keys():
                    raise ValidationError(
                        f'unknown property: {property_name}', path + ['(recipes)', f'[{i}]'])
                if schema['properties'][property_name]['type'] not in ['text', 'quantity', 'datetime', 'bool']:
                    raise ValidationError(
                        'unsupported type in recipe', path + ['(recipes)', f'[{i}]', property_name])
                if recipe['property_values'][property_name] is not None:
                    validate(recipe['property_values'][property_name], schema['properties']
                             [property_name], path + ['(recipes)', f'[{i}]', property_name], strict=strict)
                    if recipe['property_values'][property_name]['_type'] == 'quantity' and \
                       ...
                       (recipe['property_values'][property_name]['units'] not in schema['properties'][property_name]['units'])):
                        raise ValidationError(f'Invalid unit {recipe["property_values"][property_name]["units"]}, allowed unit(s): {schema["properties"][property_name]["units"]}', path + [
                                              '(recipes)', f'[{i}]', property_name])
                elif schema['properties'][property_name]['type'] == 'bool':
                    raise ValidationError('recipe values for type \'bool\' must not be None', path + [
                                          '(recipes)', f'[{i}]', property_name])
    ...
    for show_more_key in ['show_more', 'workflow_show_more']:
        if show_more_key in schema:
            ...
            for property_name in schema[show_more_key]:
                if property_name not in schema['properties'].keys():
                    raise ValidationError(
                        f'unknown property: {property_name}', path)

    if 'workflow_view' in schema and 'workflow_views' in schema:
        raise ValidationError(
            'workflow_view and workflow_views are mutually exclusive', path)
    ...
    for workflow_index, workflow_view in enumerate(workflow_views):
        id_keys = ['referencing_action_id', 'referenced_action_id',
                   'referencing_action_type_id', 'referenced_action_type_id']
        workflow_valid_keys = set(id_keys + ['title', 'show_action_info', 'sorting_properties',
                                  'recursion_filters', 'referenced_filter_operator', 'referencing_filter_operator'])
        if not isinstance(workflow_view, dict):
            raise ValidationError(
                f'workflow_view {workflow_index} must be a dict', path)
        for key in workflow_view.keys():
            if key not in workflow_valid_keys:
                raise ValidationError(
                    f'invalid key in workflow_view {workflow_index}: {key}', path)
        for key in id_keys:
            if key in workflow_view and not (
                ...
                raise ValidationError(
                    f'{key} in workflow_view {workflow_index} must be int, None or a list of ints', path)
        if 'title' in workflow_view:
            _validate_title_and_tooltip_in_schema(
                workflow_view, path, all_language_codes=all_language_codes, strict=strict)
        if 'show_action_info' in workflow_view and not isinstance(workflow_view['show_action_info'], bool):
            raise ValidationError(
                f'show_action_info in workflow_view {workflow_index} must be bool', path)
        if 'sorting_properties' in workflow_view and not isinstance(workflow_view['sorting_properties'], list):
            raise ValidationError(
                f'sorting_properties in workflow_view {workflow_index} must be list', path)
        ...
        _validate_filter_operators(
            ...
            filter_operator_keys=(
                'referencing_filter_operator', 'referenced_filter_operator'),
            path=path
        )
        if 'recursion_filters' in workflow_view:
            recursion_id_keys = ['referenced_action_id', 'referenced_action_type_id',
                                 'referencing_action_id', 'referencing_action_type_id']
            recursion_valid_keys = set(
                id_keys + ['max_depth', 'referenced_filter_operator', 'referencing_filter_operator'])
            recursion_filters = workflow_view['recursion_filters']
            if not isinstance(recursion_filters, dict):
                raise ValidationError(
                    f'recursion_filters in workflow_view {workflow_index} must be a dict', path)
            for key in recursion_filters.keys():
                if key not in recursion_valid_keys:
                    raise ValidationError(
                        f'invalid key in recursion_filters in workflow_view {workflow_index}: {key}', path)
            for key in recursion_id_keys:
                if key in recursion_filters and not (
                    ...
                    raise ValidationError(
                        f'{key} in recursion_filters in workflow_view {workflow_index} must be int, None or a list of ints', path)
            if 'max_depth' in recursion_filters and not isinstance(recursion_filters['max_depth'], int):
                raise ValidationError(
                    f'max_depth in recursion_filters in workflow_view {workflow_index} must be int', path)
            if 'max_depth' in recursion_filters and recursion_filters['max_depth'] <= 0:
                raise ValidationError(
                    f'max_depth in recursion_filters in workflow_view {workflow_index} must not be negative', path)
            _validate_filter_operators(
...
def _validate_filter_operators(
    ...
    for filter_operator_key in filter_operator_keys:
        if mapping.get(filter_operator_key, default_filter_operator) not in valid_filter_operators:
            raise ValidationError(f'{filter_operator_key} must be one of:' +
                                  ', '.join(map(repr, valid_filter_operators)), path)
...
def _validate_text_schema(
    ...
    :raise ValidationError: if the schema is invalid.
    """
    valid_keys = {'type', 'title', 'default', 'minLength', 'maxLength', 'choices', 'pattern', 'multiline', 'markdown', 'note', 'placeholder',
                  'dataverse_export', 'scicat_export', 'languages', 'conditions', 'may_copy', 'style', 'tooltip', 'dynamic_choices_source'}
    schema_keys = set(schema.keys())
    invalid_keys = schema_keys - valid_keys
    if invalid_keys:
        raise ValidationError(
            f'unexpected dynamic choices still keys in schema: {invalid_keys}', path)

    if 'languages' in schema:
        if schema['languages'] != 'all':
            if not isinstance(schema['languages'], list) or len(schema['languages']) == 0:
                raise ValidationError(
                    'languages must be a non-empty list of known language codes or "all"', path)
            for language in schema['languages']:
                if not isinstance(language, str) or language not in all_language_codes:
                    raise ValidationError(
                        'languages must be a list of known language codes or "all"', path)
    ...
    if 'placeholder' in schema and 'choices' in schema:
        raise ValidationError(
            'placeholder cannot be used together with choices', path)
    ...
    if 'placeholder' in schema and isinstance(schema['placeholder'], dict):
        if 'en' not in schema['placeholder']:
            raise ValidationError(
                'placeholder must include an english translation', path)
        for lang_code in schema['placeholder'].keys():
            if lang_code not in all_language_codes:
                raise ValidationError(
                    'placeholder must only contain known languages', path)
        for placeholder_text in schema['placeholder'].values():
            if not isinstance(placeholder_text, str):
                raise ValidationError(
                    'placeholder must only contain text', path)
    ...
    if 'minLength' in schema and 'maxLength' in schema and schema['maxLength'] < schema['minLength']:
        raise ValidationError(
            'maxLength must not be less than minLength', path)
    ...
    if 'choices' in schema:
        ...
        for i, choice in enumerate(schema['choices']):
            if not isinstance(choice, str) and not isinstance(choice, dict):
                raise ValidationError(
                    'choice must be str or dict', path + [str(i)])
            if choice_type is not None and type(choice) is not choice_type:
                raise ValidationError(
                    'choices must be either all str or all dict', path + [str(i)])
            choice_type = type(choice)
            if isinstance(choice, dict):
                if 'en' not in choice:
                    raise ValidationError(
                        'choice must include an english translation', path + [str(i)])
                for lang_code in choice.keys():
                    if lang_code not in all_language_codes:
                        raise ValidationError(
                            'choice must only contain known languages', path + [str(i)])
                for choice_text in choice.values():
                    if not isinstance(choice_text, str):
                        raise ValidationError(
                            'choice must only contain text', path + [str(i)])
                    if choice_text.isspace():
                        raise ValidationError(
                            'choice must contain more than whitespace', path + [str(i)])
            else:
                if choice.isspace():
                    raise ValidationError(
                        'choice must contain more than whitespace', path + [str(i)])
    ...
    if 'choices' in schema and 'default' in schema:
        if schema['default'] not in schema['choices']:
            raise ValidationError(
                'default must only contain a valid choice', path)
        ...
        for lang_code in schema['default'].keys():
            if lang_code not in allowed_language_codes:
                raise ValidationError(
                    'default must only contain allowed languages', path)
    ...
    if 'pattern' in schema:
        try:
            ...
            raise ValidationError(
                'pattern is no valid regular expression', path)
    ...
    if schema.get('markdown') and schema.get('multiline'):
        raise ValidationError(
            'text may not both be multiline and markdown', path)
    if 'choices' in schema and schema.get('multiline'):
        raise ValidationError(
            'text may not both be multiline and have choices', path)
    if 'choices' in schema and schema.get('markdown'):
        raise ValidationError(
            'text may not both be markdown and have choices', path)
    if 'dataverse_export' in schema and not isinstance(schema['dataverse_export'], bool):
        raise ValidationError('dataverse_export must be True or False', path)
    if 'dataverse_export' in schema and not schema['dataverse_export'] and path == ['name']:
        raise ValidationError(
            'dataverse_export must be True for the object name', path)
    if 'scicat_export' in schema and not isinstance(schema['scicat_export'], bool):
        raise ValidationError('scicat_export must be True or False', path)
    if 'scicat_export' in schema and not schema['scicat_export'] and path == ['name']:
        raise ValidationError(
            'scicat_export must be True for the object name', path)
    _validate_note_in_schema(
        schema, path, all_language_codes=all_language_codes, strict=strict)
...
def _validate_datetime_schema(
    ...
    valid_keys = {'type', 'title', 'default', 'note', 'dataverse_export',
                  'scicat_export', 'conditions', 'may_copy', 'style', 'tooltip'}
    schema_keys = set(schema.keys())
    invalid_keys = schema_keys - valid_keys
    if invalid_keys:
        raise ValidationError(
...
def _validate_quantity_schema(
    ...
    :raise ValidationError: if the schema is invalid.
    """
    valid_keys = {'type', 'title', 'units', 'default', 'note', 'placeholder', 'dataverse_export', 'scicat_export',
                  'conditions', 'may_copy', 'style', 'tooltip', 'display_digits', 'min_magnitude', 'max_magnitude', 'calculation'}
    ...
    if isinstance(schema['units'], str):
        ...
        for unit in schema['units']:
            if not isinstance(unit, str):
                raise ValidationError(
                    'units must be string or list of strings', path)
            try:
            ...
            if dimensionality is None:
                ...
                raise ValidationError(
                    'units must be for same dimensionality', path)
    ...
    if 'min_magnitude' in schema and 'max_magnitude' in schema and schema['min_magnitude'] > schema['max_magnitude']:
        raise ValidationError(
            'max_magnitude must be greater than or equal to min_magnitude', path)
    if 'default' in schema:
        if isinstance(schema['default'], (float, int)):
            ...
            if 'min_magnitude' in schema and schema['min_magnitude'] > schema['default']:
                raise ValidationError(
                    'default must be greater than or equal to min_magnitude', path)
            if 'max_magnitude' in schema and schema['max_magnitude'] < schema['default']:
                raise ValidationError(
                    'default must be less than or equal to max_magnitude', path)
    ...
    if 'display_digits' in schema and (type(schema['display_digits']) is not int or schema['display_digits'] < 0):
        raise ValidationError(
            'display_digits must be a non-negative int', path)
    if strict:
    ...
    if 'calculation' in schema:
        calculations.validate_calculation(
            schema['calculation'], root_schema, path)
    _validate_note_in_schema(
        schema, path, all_language_codes=all_language_codes, strict=strict)
...
def _validate_object_reference_schema(
    ...
    :raise ValidationError: if the schema is invalid.
    """
    valid_keys = {'type', 'title', 'note', 'action_type_id', 'action_id', 'filter_operator',
                  'dataverse_export', 'scicat_export', 'conditions', 'may_copy', 'style', 'tooltip', 'default'}
    required_keys = {'type', 'title'}
    schema_keys = set(schema.keys())
    invalid_keys = schema_keys - valid_keys
    if invalid_keys:
    ...
    if 'action_type_id' in schema and not (
        ...
        raise ValidationError(
            'action_type_id must be int, dict, None or a list of ints or dicts', path)
    if 'action_id' in schema and not (
        ...
        raise ValidationError(
            'action_id must be int, dict, None or a list of ints or dicts', path)
    _validate_filter_operators(
        mapping=schema,
...
def _validate_notebook_templates(notebook_templates: typing.Any) -> None:
    ...
    if not isinstance(notebook_templates, list):
        raise ValidationError('notebookTemplates must be a list', [
                              'notebookTemplates'])
    ...
    for notebook_index, notebook_template in enumerate(notebook_templates):
        ...
        if invalid_keys:
            raise ValidationError(
                f'unexpected keys in notebook template: {invalid_keys}', path)
        missing_keys = required_keys - schema_keys
        if missing_keys:
            raise ValidationError(
                f'missing keys in notebook template: {missing_keys}', path)
        ...
        if not notebook_template['url'].endswith('.ipynb'):
            raise ValidationError(
                'notebook template url must end with .ipynb', path)
...
def _validate_user_schema(
    ...
    """
    valid_keys = {'type', 'title', 'note', 'dataverse_export',
                  'scicat_export', 'default', 'conditions', 'may_copy', 'style', 'tooltip'}
    required_keys = {'type', 'title'}
    schema_keys = set(schema.keys())
    invalid_keys = schema_keys - valid_keys
    if invalid_keys:
...
def _validate_plotly_chart_schema(
    ...
    """
    valid_keys = {'type', 'title', 'note', 'dataverse_export',
                  'conditions', 'may_copy', 'style', 'tooltip'}
...
def _validate_timeseries_schema(
    ...
    :raise ValidationError: if the schema is invalid.
    """
    valid_keys = {'type', 'title', 'units', 'note', 'dataverse_export', 'scicat_export',
                  'conditions', 'may_copy', 'style', 'tooltip', 'display_digits', 'statistics'}
    required_keys = {'type', 'title', 'units'}
    schema_keys = set(schema.keys())
    invalid_keys = schema_keys - valid_keys
    if invalid_keys:
    ...
    if 'statistics' in schema:
        valid_statistics_keys = {'average', 'stddev',
        ...
        if not all(statistics_key in valid_statistics_keys for statistics_key in schema['statistics']):
            raise ValidationError(
                f'statistics keys each have to be one of the following: {valid_statistics_keys}', path)
    _validate_note_in_schema(
        schema, path, all_language_codes=all_language_codes, strict=strict)
...
def _validate_file_schema(
    ...
    :raise ValidationError: if the schema is invalid.
    """
    valid_keys = {'type', 'title', 'note', 'conditions',
                  'style', 'tooltip', 'extensions', 'preview'}
    required_keys = {'type', 'title'}
    schema_keys = set(schema.keys())
    invalid_keys = schema_keys - valid_keys
    if invalid_keys:
    ...
    if 'extensions' in schema:
        if not isinstance(schema['extensions'], list) or not schema['extensions'] or not all(isinstance(extension, str) and len(extension) > 1 and extension.startswith('.') for extension in schema['extensions']):
            raise ValidationError(
                'extensions must be a list of file extensions', path)
    if 'preview' in schema and not isinstance(schema['preview'], bool):
        raise ValidationError(
            'preview must be a list of file extensions', path)
    _validate_note_in_schema(
        schema, path, all_language_codes=all_language_codes, strict=strict)
